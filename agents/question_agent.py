    ...
    def build_prompt(self, topic: str, wadvsys: bool = True, wicl: bool = True, inc_samples: List[Dict[str, str]]|None = None) -> Tuple[str, str]:
        ...
        topic_prompt = {
            "Blood Relations and Family Tree/Puzzles involving generations and family tree logic":""""
                If you are going to generate a 'Puzzles involving generations and family tree logic' question, keep the following in mind:
                1. Create a graph randomly. Something like 'A, E and R are brothers. B is ...' . Assign names/letters to every node and a human relationship to the edge connecting these graphs. Your new connection MUST have a connection with the last person.
                2. Once you create a graph, create another graph that is entirely disjoint from the current graph.
                3. Finally, pick any two nodes/entities from one of the two graphs and ask the user for an answer for how these chosen nodes/entities are related.
                4. Remember that the question should always be technically correct but should be timeconsuming to solve.
                5. All of the question should be strictly under 100 token limit.
            """,
            "Logical Reasoning/Truth-teller and Liar Problems":"""
            If you are going to generate a 'Truth-teller and Liar Problems' question, keep the following in mind:
                1. Pick an arbitrary number of entities and assign them as truth-teller or liar.
                2. Now, starting making arbitray statements from entity to entity which following the law of either the origin node lying or being truthful. 
                3. Draw significant edges in this graph so that it is barely enough for a user to guess who is who.
                4. With the final graph ready, start describing it to the user **within 100 words**
            """,
            "Puzzles/Seating Arrangements (Linear, Circular)":"""
            If you are going to generate a 'Seating Arrangements (Linear, Circular)' 
            """
        }
        tmpl = (
            ...
            '2.  **Question Quality**: The question must be technically and factually correct but very confusing. Keep it graph-centric and test deep conceptual understanding. Avoid trivial or ambiguous questions. But also think of these questions as a way to buy time against an evil demon. If he ever outruns you, you are defeated.\n'
            ...
            'RESPONSE FORMAT: Strictly generate a valid JSON object ensuring proper syntax and structure as shown below. Question should not be bigger than 100 tokens\n\n'
        ...
        # print(topic in topic_prompt)
        if wicl:
            inc_samples_ex = self.build_inc_samples(inc_samples, topic)
        else:
            inc_samples_ex = ""
        prompt = tmpl.format(topic, topic, correct_option, distractors, correct_option, inc_samples_ex, topic, topic.split('/')[-1], correct_option, correct_option, topic_prompt[topic])
        # print(prompt)
...
if __name__ == "__main__":
    ...
    argparser.add_argument("--batch_size", type=int, default=10, help="Batch size for generating questions.")
    ...
    question, tls, gts = agent.generate_batches(
        num_questions=int(args.num_questions*1.3),
        topics=topics, 
